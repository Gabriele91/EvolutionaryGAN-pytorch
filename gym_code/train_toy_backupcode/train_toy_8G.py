        ...
        if n_updates == 0:
            for can_i in range(0, ncandi):
                ...
                if can_i == 0:
                    ...
                    fmb = np.append(
                        fmb, gen_imgs[0:int(batchSize/ncandi*kD), :], axis=0)
            ...
            for can_i in range(0, ncandi):
                for type_i in range(0, nloss):
                    ...
                    if can_i*nloss + type_i < ncandi:
                        ...
                        gen_new_params[idx] = lasagne.layers.get_all_param_values(
                            generator)
                        fake_rate[idx] = frr
                        g_imgs_old[idx*ntf:(idx+1)*ntf, :] = gen_imgs
                        fmb[int(idx*batchSize/ncandi*kD):int((idx+1)*batchSize/ncandi*kD), :] = \
                        ...
                        if min(fr_com) < 0:
                            ...
                            fmb[int(idr*batchSize/ncandi*kD):int((idr+1)*batchSize/ncandi*kD), :] = \
                                gen_imgs[0:int(batchSize/ncandi*kD), :]
